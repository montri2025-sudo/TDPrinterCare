...
_CHECK_CONSTRAINTS = [
    ("Users", "chk_users_role",
     "role IN ('customer','admin','technician','technician_lead','manager','sales')"),
...
def _migrate_schema(conn: _Conn):
    ...
    if not _column_exists(conn, "Restock_Orders", "payment_confirmed_at"):
        print("[init_db] migrate: เพิ่มคอลัมน์ Restock_Orders.payment_confirmed_at/payment_confirmed_by "
              "(ยืนยันรับชำระเงินค่าสินค้าจากศูนย์บริการ เพื่อออกใบกำกับภาษี — คำสั่งซื้อเดิมทั้งหมดก่อนหน้านี้จะยังไม่ยืนยัน)")
        conn.execute("ALTER TABLE Restock_Orders ADD COLUMN payment_confirmed_at TEXT")
        conn.execute("ALTER TABLE Restock_Orders ADD COLUMN payment_confirmed_by INT")
        conn.execute(
            "ALTER TABLE Restock_Orders ADD CONSTRAINT fk_restock_orders_payment_confirmed_by "
