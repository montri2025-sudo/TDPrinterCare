...
# ---------------------------------------------------------------------------
# Pure Python (wsgiref) + Jinja2 app image. Talks to a PostgreSQL database over
# the network (see docker-compose.yml for the paired postgres service + env
# vars: PG_HOST/PORT/USER/PASSWORD/DATABASE). Uses psycopg2-binary (precompiled
# wheel, no extra apt packages needed on python:3.11-slim for the DB driver).
#
# This Dockerfile expects the app source (app.py, db.py, odoo_client.py, schema.sql,
...
# App source
COPY app.py db.py odoo_client.py schema.sql ./
COPY templates ./templates
COPY static ./static

# Uploaded photos/videos live here — mount as a volume at runtime so they
# survive container restarts/rebuilds (the database itself lives in PostgreSQL,
