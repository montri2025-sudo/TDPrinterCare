...
def user_can_view_ticket(conn, user, ticket_id):
    ...
    if user["role"] == "admin":
        return True
    if user["role"] in ("manager", "technician_lead"):
...
def require_center_access(user, resource_center_id):
    """สำหรับ manager และ technician_lead (หัวหน้าช่าง): บล็อกการเข้าถึง/แก้ไขข้อมูลของศูนย์บริการอื่น (403) —
    หัวหน้าช่างดูแลทีมช่าง/ตั๋วซ่อมเฉพาะศูนย์ของตัวเองเหมือนผู้จัดการ
    resource_center_id=None ถือเป็นข้อมูลกลาง/ใช้ร่วมกันทุกสาขา อนุญาตเสมอ ไม่จำกัดผลกับ admin"""
    if user["role"] in ("manager", "technician_lead") and resource_center_id is not None:
...
def _user_can_view_activity_category(user, category):
    ...
    if category == "technical":
        return user["role"] in ("technician", "technician_lead")
    return False
...
def _ticket_detail_url_for(user, ticket_id):
    ...
    technician_lead (หัวหน้าช่าง) ใช้หน้าเดียวกับ admin/manager เพราะดูแลตั๋วทั้งศูนย์ ไม่ใช่แค่ตั๋วที่รับมอบหมายเอง"""
    if user["role"] in ("admin", "manager", "technician_lead"):
...
def _center_overview_map(conn, center_ids):
    ...
    for cid in center_ids:
        ...
        technicians = conn.execute(
            "SELECT name FROM Users WHERE role IN ('technician','technician_lead') AND center_id=? AND is_active=1 ORDER BY name",
            (cid,),
        ).fetchall()
        queue = conn.execute(
...
COMMISSION_BONUS_CASE_RANK = 11  # เริ่มคิดค่าคอมมิชชั่นพิเศษจากเคสที่ปิดงานได้เป็นลำดับที่ 11 ขึ้นไปในรอบบิล
COMMISSION_BONUS_RATE = 0.5      # ค่าคอมมิชชั่นพิเศษ = 50% ของค่าแรงในเคสนั้น

# ลูกค้าภายใน (เครื่องทดสอบ/สาธิตของบริษัทเอง) — ตั๋วซ่อมของลูกค้ารายนี้จะไม่ถูกนับเป็น "เคส" ของช่างเลย
# ในทุกที่ที่มีการนับจำนวนเคสของช่าง/หัวหน้าช่าง (รายงานส่วนตัว, รายงานผลงานของแอดมิน/ผู้จัดการ,
# workload บน Dashboard, และการคำนวณค่าคอมมิชชั่นพิเศษ) เพราะไม่ใช่งานซ่อมของลูกค้าจริง
EXCLUDED_CASE_COUNT_CUSTOMER_NAME = "TDPrinter"


def commission_period_range(environ):
    """ช่วงรอบตัดค่าคอมมิชชั่นพิเศษของช่าง/หัวหน้าช่าง — ตัดรอบทุกครึ่งเดือน: งวดที่ 1 (วันที่ 1-15)
    และงวดที่ 2 (วันที่ 16-สิ้นเดือน) อ่านจาก query string ?year=YYYY&month=M&half=1|2 —
    ค่าเริ่มต้นคืองวดปัจจุบันของวันนี้ คืนค่า (from_sql, to_sql, label, year, month, half)"""
    import datetime
    import calendar
    qs = parse_qs(environ.get("QUERY_STRING", ""))
    today = datetime.date.today()

    def _int(raw, fallback, lo, hi):
        try:
            v = int(raw)
    ...
    year = _int(qs.get("year", [""])[0], today.year, 2000, 2100)
    month = _int(qs.get("month", [""])[0], today.month, 1, 12)
    default_half = 1 if today.day <= 15 else 2
    half = _int(qs.get("half", [""])[0], default_half, 1, 2)

    last_day = calendar.monthrange(year, month)[1]
    if half == 1:
        from_d = datetime.date(year, month, 1)
        to_d = datetime.date(year, month, 15)
    else:
        from_d = datetime.date(year, month, 16)
        to_d = datetime.date(year, month, last_day)

    from_sql = from_d.strftime("%Y-%m-%d") + " 00:00:00"
    to_sql = to_d.strftime("%Y-%m-%d") + " 23:59:59"
    label = f"{from_d.strftime('%d/%m/%Y')} - {to_d.strftime('%d/%m/%Y')}"
    return from_sql, to_sql, label, year, month, half


def _technician_commission_rows(conn, from_sql, to_sql, tech_user_id=None, center_id=None):
    """คำนวณค่าคอมมิชชั่นพิเศษของช่าง/หัวหน้าช่าง ต่อรอบบิล (ครึ่งเดือน) — เรียงลำดับตั๋วที่ปิดงานได้
    (Resolved/Closed) ของช่างแต่ละคนตามเวลาปิดงาน (closed_at) ภายในรอบนี้ เคสที่ปิดได้เป็นลำดับที่
    COMMISSION_BONUS_CASE_RANK ขึ้นไป จะได้ค่าคอมมิชชั่นพิเศษ = COMMISSION_BONUS_RATE ของค่าแรงรวมในเคสนั้น
    (ค่าแรง = SUM(Service_Logs.labor_fee) ที่ approval_status IN ('auto','approved') ของตั๋วนั้น —
    นิยาม "ค่าแรงที่เรียกเก็บได้จริง" เดียวกับที่ build_invoice() ใช้)
    ใช้ได้ทั้งแบบดูทั้งทีม (tech_user_id=None) และดูช่างคนเดียว (ใช้ตอนออกบิล)
    คืนค่า list ของ dict ต่อช่างหนึ่งคน: {user_id, name, role, center_name, closed_count,
    bonus_case_count, total_labor, total_commission, cases: [...]}"""
    where = ["u.role IN ('technician','technician_lead')", "u.is_active = 1"]
    params = []
    if tech_user_id:
        where.append("u.user_id = ?")
        params.append(tech_user_id)
    if center_id:
        where.append("u.center_id = ?")
        params.append(center_id)
    techs = conn.execute(
        f"""SELECT u.user_id, u.name, u.role, sc.name AS center_name FROM Users u
            LEFT JOIN Service_Centers sc ON sc.center_id = u.center_id
            WHERE {' AND '.join(where)} ORDER BY u.name""",
        tuple(params),
    ).fetchall()
    ...
    for tech in techs:
        tickets = conn.execute(
            """SELECT t.ticket_id, t.closed_at, d.model, c.name AS customer_name FROM Tickets t
               JOIN Devices d ON d.device_sn = t.device_sn
               JOIN Customers c ON c.customer_id = d.customer_id
               WHERE t.assigned_tech_id=? AND t.status='Resolved/Closed'
                     AND t.closed_at BETWEEN ? AND ? AND c.name != ?
               ORDER BY t.closed_at, t.ticket_id""",
            (tech["user_id"], from_sql, to_sql, EXCLUDED_CASE_COUNT_CUSTOMER_NAME),
        ).fetchall()
        cases = []
        total_labor = 0.0
        ...
        for idx, t in enumerate(tickets, start=1):
            labor_row = conn.execute(
                """SELECT COALESCE(SUM(labor_fee), 0) AS s FROM Service_Logs
                   WHERE ticket_id=? AND approval_status IN ('auto','approved')""",
                (t["ticket_id"],),
            ).fetchone()
            labor_total = float(labor_row["s"] or 0)
            bonus = 0.0
            if idx >= COMMISSION_BONUS_CASE_RANK:
                bonus = round(labor_total * COMMISSION_BONUS_RATE, 2)
                bonus_case_count += 1
            total_labor += labor_total
            total_commission += bonus
            cases.append({
                "rank": idx, "ticket_id": t["ticket_id"], "closed_at": t["closed_at"],
                "model": t["model"], "customer_name": t["customer_name"],
                "labor_total": labor_total, "bonus": bonus,
            })
        rows.append({
            "user_id": tech["user_id"], "name": tech["name"], "role": tech["role"],
            "center_name": tech["center_name"], "closed_count": len(tickets),
            "bonus_case_count": bonus_case_count, "total_labor": round(total_labor, 2),
            "total_commission": round(total_commission, 2), "cases": cases,
        })
    return rows
...
GUIDE_SECTIONS = [
    ...
    {"slug": "technician", "title": "ช่างซ่อม", "icon": "🔧",
     "subtitle": "รับงาน บันทึกการซ่อม เบิกอะไหล่ ออกใบเสนอราคา/ใบแจ้งหนี้", "roles": {"admin", "technician", "technician_lead"}},
...
def _user_can_access_device(user, device):
    ...
    if not user or not device:
        return False
    if user["role"] in ("admin", "manager", "technician", "technician_lead", "sales"):
        return True
    if user["role"] == "customer":
    ...
    if scope_center:
        ...
        workload = conn.execute(
            """SELECT u.name, sc.name AS center_name, COUNT(t.ticket_id) open_tickets FROM Users u
               ...
               LEFT JOIN Tickets t ON t.assigned_tech_id = u.user_id AND t.status != 'Resolved/Closed'
                    AND t.device_sn NOT IN (
                        SELECT dd.device_sn FROM Devices dd JOIN Customers cc ON cc.customer_id = dd.customer_id
                        WHERE cc.name = ?
                    )
               WHERE u.role IN ('technician','technician_lead') AND u.center_id = ? GROUP BY u.user_id, u.name, sc.name""",
            (EXCLUDED_CASE_COUNT_CUSTOMER_NAME, scope_center),
        ).fetchall()
        ...
        workload = conn.execute(
            """SELECT u.name, sc.name AS center_name, COUNT(t.ticket_id) open_tickets FROM Users u
               ...
               WHERE u.role IN ('technician','technician_lead') GROUP BY u.user_id, u.name, sc.name""",
            (EXCLUDED_CASE_COUNT_CUSTOMER_NAME,),
        ).fetchall()
        low_stock = conn.execute(
            """SELECT p.*, sc.name AS center_name FROM Spare_Parts p
               LEFT JOIN Service_Centers sc ON sc.center_id = p.center_id
...
def admin_board(environ, m, conn, user):
    require_login(user, "admin", "manager", "technician_lead")
    is_manager = user["role"] in ("manager", "technician_lead")
    ...
    if is_manager:
        ...
        techs = conn.execute(
            "SELECT * FROM Users WHERE role IN ('technician','technician_lead') AND center_id = ?", (scope_center,)
        ).fetchall()
    else:
        tickets = conn.execute(
            """SELECT t.*, d.model, c.name AS customer_name, u.name AS tech_name, sc.name AS center_name
        ...
        techs = conn.execute("SELECT * FROM Users WHERE role IN ('technician','technician_lead')").fetchall()
...
def staff_new_ticket_form(environ, m, conn, user):
    require_login(user, "admin", "manager", "sales", "technician_lead")
...
def admin_ticket_detail(environ, m, conn, user):
    ...
    if user["role"] in ("manager", "technician_lead"):
        techs = conn.execute(
            "SELECT * FROM Users WHERE role IN ('technician','technician_lead') AND is_active=1 AND center_id=?",
            (user.get("center_id"),),
        ).fetchall()
    else:
        techs = conn.execute(
            "SELECT * FROM Users WHERE role IN ('technician','technician_lead') AND is_active=1"
        ).fetchall()
    quotes = get_quotes_for_ticket(conn, ticket_id)
    invoice_items, invoice_total = build_invoice(conn, ticket_id)
    payments = get_payments_for_ticket(conn, ticket_id)
...
def admin_customers(environ, m, conn, user):
    ...
    # technician_lead (หัวหน้าช่าง) เพิ่ม/แก้ไขลูกค้าได้เหมือน manager (เทมเพลตซ่อนปุ่มเฉพาะ role 'sales' เท่านั้น
    # จึงเห็นปุ่มแก้ไข/ฟอร์มแก้ไขลูกค้าได้ปกติ ไม่ต้องแก้เทมเพลตเพิ่ม)
    require_login(user, "admin", "manager", "sales", "technician_lead")
...
def _restock_orders_with_items(conn, where_sql="", params=()):
    ...
    orders = conn.execute(
        f"""SELECT ro.*, sc.name AS center_name, u.name AS requested_by_name, pc.name AS payment_confirmed_by_name
            FROM Restock_Orders ro
            JOIN Service_Centers sc ON sc.center_id = ro.center_id
            JOIN Users u ON u.user_id = ro.requested_by
            LEFT JOIN Users pc ON pc.user_id = ro.payment_confirmed_by
            {where_sql}
...
def admin_restock_order_detail(environ, m, conn, user):
    require_login(user, "admin")
    order_id = int(m.group(1))
    order = _restock_order_with_names(conn, order_id)
    if not order:
        raise HttpError(404, "ไม่พบคำสั่งซื้อนี้")
    items = conn.execute(
        """SELECT roi.*, sp.part_name FROM Restock_Order_Items roi
...
def _restock_order_with_names(conn, order_id):
    """โหลดคำสั่งซื้อ 1 รายการ พร้อมชื่อศูนย์บริการปลายทาง/ผู้สั่งซื้อ/ผู้ยืนยันรับชำระ (ถ้ามี) — ใช้ร่วมกัน
    ทั้งหน้ารายละเอียดและหน้าพิมพ์เอกสาร (ใบเสนอราคา/ใบกำกับภาษี)"""
    return conn.execute(
        """SELECT ro.*, sc.name AS center_name, sc.address AS center_address, sc.phone AS center_phone,
                  sc.tax_id AS center_tax_id, u.name AS requested_by_name, pc.name AS payment_confirmed_by_name
           FROM Restock_Orders ro
           JOIN Service_Centers sc ON sc.center_id = ro.center_id
           JOIN Users u ON u.user_id = ro.requested_by
           LEFT JOIN Users pc ON pc.user_id = ro.payment_confirmed_by
...
@route("GET", r"/admin/restock-orders/(\d+)/quote/print")
def admin_restock_quote_print(environ, m, conn, user):
    """ใบเสนอราคา — เอกสารข้อมูลอย่างเดียว ไม่ใช่การยืนยันรับชำระเงิน พิมพ์ได้ทุกสถานะยกเว้นคำสั่งซื้อที่ยกเลิกแล้ว
    รองรับ checkbox "รวม VAT" ส่งมาเป็น query string ?vat=1 (GET form เหมือน sales_quote_print) — ถ้าไม่เลือก
    จะไม่คำนวณ/ไม่แสดงแถวภาษีมูลค่าเพิ่มเลย ผู้ซื้อในเอกสารนี้คือศูนย์บริการปลายทางของคำสั่งซื้อ (ไม่มีแนวคิด
    "ลูกค้า" แบบบิลขายปกติ เพราะเป็นการซื้อขายภายในระหว่าง HQ กับศูนย์บริการ)"""
    require_login(user, "admin")
    order_id = int(m.group(1))
    order = _restock_order_with_names(conn, order_id)
    if not order:
    ...
    if order["status"] == "cancelled":
        raise HttpError(404, "คำสั่งซื้อนี้ถูกยกเลิกแล้ว พิมพ์เอกสารนี้ไม่ได้")
    qs = parse_qs(environ.get("QUERY_STRING", ""))
    ...
    subtotal = round(sum((it["quantity_requested"] or 0) * (it["unit_price"] or 0) for it in items), 2)
    if include_vat:
    ...
    hq = _get_hq_center(conn)
    return render("restock_quote_print.html", order=order, items=items, subtotal=subtotal, vat=vat,
                  grand_total=grand_total, include_vat=include_vat, vat_rate_percent=int(db.VAT_RATE * 100), hq=hq)


@route("POST", r"/admin/restock-orders/(\d+)/confirm-payment")
def admin_restock_confirm_payment(environ, m, conn, user):
    """ยืนยันว่าศูนย์บริการนี้ชำระค่าสินค้าคำสั่งซื้อนี้แล้ว — ล็อกไว้ไม่ให้ยืนยันซ้ำ/เปลี่ยนแปลงได้ (กดซ้ำแค่พาไปหน้า
    พิมพ์ใบกำกับภาษีเดิม) จากนั้นพาไปหน้าพิมพ์ใบกำกับภาษีทันที เหมือนรูปแบบ sales_order_confirm_payment"""
    require_login(user, "admin")
    order_id = int(m.group(1))
    order = _get_restock_order_or_404(conn, order_id)
    if order["status"] == "cancelled":
        raise HttpError(400, "คำสั่งซื้อนี้ถูกยกเลิกแล้ว ยืนยันรับชำระเงินไม่ได้")
    if not order["payment_confirmed_at"]:
        conn.execute(
            "UPDATE Restock_Orders SET payment_confirmed_at=?, payment_confirmed_by=? WHERE order_id=?",
            (db.now(), user["user_id"], order_id),
        )
        conn.commit()
    raise Redirect(f"/admin/restock-orders/{order_id}/tax-invoice")


@route("GET", r"/admin/restock-orders/(\d+)/tax-invoice")
def admin_restock_tax_invoice_print(environ, m, conn, user):
    """ใบกำกับภาษี — ออกได้เฉพาะคำสั่งซื้อที่ยืนยันรับชำระเงินแล้วเท่านั้น (payment_confirmed_at ไม่ว่าง)
    รวม VAT เสมอ (ไม่มีตัวเลือกปิด VAT เหมือนใบเสนอราคา เพราะเป็นเอกสารทางภาษีที่ต้องแสดงยอดจริง)"""
    require_login(user, "admin")
    order_id = int(m.group(1))
    order = _restock_order_with_names(conn, order_id)
    if not order:
        raise HttpError(404, "ไม่พบคำสั่งซื้อนี้")
    if not order["payment_confirmed_at"]:
        raise HttpError(404, "คำสั่งซื้อนี้ยังไม่ได้ยืนยันรับชำระเงิน ออกใบกำกับภาษีไม่ได้")
    items = conn.execute(
        """SELECT roi.*, sp.part_name FROM Restock_Order_Items roi
           JOIN Spare_Parts sp ON sp.part_sku = roi.part_sku
           WHERE roi.order_id=? ORDER BY roi.item_id""",
    ...
    grand_total = round(subtotal + vat, 2)
    hq = _get_hq_center(conn)
    return render("restock_tax_invoice_print.html", order=order, items=items, subtotal=subtotal, vat=vat,
                  grand_total=grand_total, vat_rate_percent=int(db.VAT_RATE * 100), hq=hq)
...
def sharespace_browse(environ, m, conn, user):
    ...
    require_login(user, "admin", "manager", "sales", "technician", "technician_lead")
    qs = parse_qs(environ.get("QUERY_STRING", ""))
    today = db.now()[:10]
    ...
    elif user["role"] in ("technician", "technician_lead"):
...
def admin_users_new(environ, m, conn, user):
    ...
    if not username or not password or not name or role not in ("customer", "admin", "technician", "technician_lead", "manager", "sales"):
...
def admin_users_edit(environ, m, conn, user):
    ...
    if role not in ("customer", "admin", "technician", "technician_lead", "manager", "sales") or not name:
        raise Redirect("/admin/users")
...
def tech_tasks(environ, m, conn, user):
    require_login(user, "technician", "technician_lead")
    tickets = conn.execute(
...
def tech_report(environ, m, conn, user):
    ...
    tickets = conn.execute(
        """SELECT t.*, d.model, c.name AS customer_name, sc.name AS center_name FROM Tickets t
           ...
           JOIN Customers c ON c.customer_id = d.customer_id
           LEFT JOIN Service_Centers sc ON sc.center_id = t.center_id
           WHERE t.assigned_tech_id=? AND c.name != ?
             AND ((t.created_at BETWEEN ? AND ?) OR (t.closed_at BETWEEN ? AND ?))
           ORDER BY t.created_at DESC""",
        (user["user_id"], EXCLUDED_CASE_COUNT_CUSTOMER_NAME, from_sql, to_sql, from_sql, to_sql),
    ).fetchall()
    ...
    # ค่าคอมมิชชั่นพิเศษของตัวเองในงวดปัจจุบัน (ครึ่งเดือน) — แสดงเป็นการ์ดสรุปพร้อมลิงก์พิมพ์บิล
    # ถ้ามีเคสที่ได้ค่าคอมมิชชั่น (ไม่ผูกกับช่วงวันที่ที่เลือกกรองด้านบน ซึ่งเป็นรายเดือน คนละรอบกับค่าคอม)
    commission_from_sql, commission_to_sql, commission_period_label, commission_year, commission_month, \
        commission_half = commission_period_range({})
    commission_row = _technician_commission_rows(
        conn, commission_from_sql, commission_to_sql, tech_user_id=user["user_id"]
    )
    commission_row = commission_row[0] if commission_row else None
    ...
    return render(
        ...
        commission_row=commission_row, commission_period_label=commission_period_label,
        commission_year=commission_year, commission_month=commission_month, commission_half=commission_half,
        bonus_case_rank=COMMISSION_BONUS_CASE_RANK, bonus_rate_percent=int(COMMISSION_BONUS_RATE * 100),
    )
...
def tech_ticket_log(environ, m, conn, user):
    require_login(user, "admin", "technician", "technician_lead")
    ticket_id = int(m.group(1))
    if user["role"] in ("technician", "technician_lead"):
        owned = conn.execute(
...
def tech_quote_form(environ, m, conn, user):
    ...
    หัวหน้าช่าง (technician_lead) สร้างได้ทุกตั๋วในศูนย์ของตัวเอง ไม่จำกัดแค่ตั๋วที่รับมอบหมายให้ตัวเอง"""
    require_login(user, "admin", "technician", "technician_lead")
    ticket_id = int(m.group(1))
    t = conn.execute(
...
def sales_order_detail(environ, m, conn, user):
    ...
    edit_centers, edit_staff, customers, customers_for_js = [], [], [], []
    if user["role"] in ("admin", "manager") and not order["cancelled_at"]:
        edit_centers = _sales_edit_center_choices(conn, user)
        edit_staff = _sales_edit_staff_choices(conn, user)
        customers = conn.execute("SELECT * FROM Customers ORDER BY name").fetchall()
        customers_for_js = _customers_for_js(customers)
    ...
    return render("sales_order_detail.html", user=user, order=order, items=items,
                   ...
                   edit_centers=edit_centers, edit_staff=edit_staff,
                   customers=customers, customers_for_js=customers_for_js,
                   sales_channels=SALES_CHANNELS)
...
def _sales_edit_center_choices(conn, user):
    """รายชื่อศูนย์บริการที่เลือกได้ในฟอร์มแก้ไขข้อมูลบิล — admin เลือกได้ทุกศูนย์ที่เปิดขายสินค้า,
    manager เห็นเฉพาะศูนย์ของตัวเอง (ตาม require_center_access ที่บังคับใช้ฝั่ง route อยู่แล้ว)"""
    if user["role"] == "manager":
        return conn.execute("SELECT * FROM Service_Centers WHERE center_id=?", (user.get("center_id"),)).fetchall()
    return conn.execute("SELECT * FROM Service_Centers WHERE sells_products=1 ORDER BY name").fetchall()


def _sales_edit_staff_choices(conn, user):
    """รายชื่อพนักงานที่เลือกเป็น "พนักงานขาย" ของบิลได้ในฟอร์มแก้ไข — admin เห็นพนักงานทุกศูนย์,
    manager เห็นเฉพาะพนักงานศูนย์ตัวเอง + แอดมินส่วนกลาง"""
    if user["role"] == "manager":
        return conn.execute(
            """SELECT u.*, sc.name AS center_name FROM Users u
               LEFT JOIN Service_Centers sc ON sc.center_id = u.center_id
               WHERE u.is_active=1 AND u.role IN ('admin','manager','sales')
                 AND (u.role='admin' OR u.center_id=?)
               ORDER BY u.name""",
            (user.get("center_id"),),
        ).fetchall()
    return conn.execute(
...
@route("POST", r"/sales/order/(\d+)/edit")
def sales_order_edit(environ, m, conn, user):
    """แก้ไขข้อมูลหัวบิล (ศูนย์บริการ/พนักงานขาย/วันที่/ลูกค้า/ช่องทางการขาย) — เฉพาะแอดมิน/ผู้จัดการ
    แก้ไขได้ไม่ว่าจะยืนยันรับชำระเงินแล้วหรือยัง แต่แก้ไขไม่ได้ถ้าบิลถูกยกเลิกแล้ว"""
    require_login(user, "admin", "manager")
    order_id = int(m.group(1))
    order = _get_sale_order_or_404(conn, order_id)
    require_center_access(user, order["center_id"])
    if order["cancelled_at"]:
        raise HttpError(400, "บิลนี้ถูกยกเลิกแล้ว แก้ไขข้อมูลไม่ได้")

    form = parse_post(environ)

    try:
        center_id = int(form.get("center_id") or "")
    except ValueError:
        raise Redirect(f"/sales/order/{order_id}")
    center = conn.execute("SELECT * FROM Service_Centers WHERE center_id=?", (center_id,)).fetchone()
    if not center:
        raise HttpError(404, "ไม่พบศูนย์บริการนี้")
    require_center_access(user, center_id)  # กันไม่ให้ manager ย้ายบิลไปศูนย์อื่นที่ไม่มีสิทธิ์เข้าถึง

    try:
        sold_by = int(form.get("sold_by") or "")
    except ValueError:
        raise Redirect(f"/sales/order/{order_id}")
    staff = conn.execute(
        "SELECT * FROM Users WHERE user_id=? AND role IN ('admin','manager','sales') AND is_active=1",
        (sold_by,),
    ).fetchone()
    if not staff:
        raise HttpError(400, "พนักงานขายที่เลือกไม่ถูกต้อง")
    ...
    created_at_raw = (form.get("created_at") or "").strip()
    if created_at_raw:
        normalized = created_at_raw.replace("T", " ")
        if len(normalized) == 16:  # "YYYY-MM-DD HH:MM" จาก <input type="datetime-local"> ไม่มีวินาที
            normalized += ":00"
        try:
            datetime.datetime.strptime(normalized, "%Y-%m-%d %H:%M:%S")
            created_at = normalized
        except ValueError:
            pass  # รูปแบบวันที่ไม่ถูกต้อง — คงค่าเดิมไว้ ไม่ล้มทั้งฟอร์ม

    conn.execute(
        "UPDATE Sales_Orders SET center_id=?, sold_by=?, customer_id=?, created_at=?, channel=? WHERE order_id=?",
        (center_id, sold_by, customer_id, created_at, channel, order_id),
    )
    conn.commit()
    raise Redirect(f"/sales/order/{order_id}")
        ...
        tech_rows = conn.execute(
            """SELECT u.user_id, u.name,
                 (SELECT COUNT(*) FROM Tickets t
                    JOIN Devices d ON d.device_sn = t.device_sn
                    JOIN Customers c ON c.customer_id = d.customer_id
                    WHERE t.assigned_tech_id = u.user_id AND t.created_at BETWEEN ? AND ?
                          AND c.name != ?) AS assigned_count,
                 (SELECT COUNT(*) FROM Tickets t
                    JOIN Devices d ON d.device_sn = t.device_sn
                    JOIN Customers c ON c.customer_id = d.customer_id
                    WHERE t.assigned_tech_id = u.user_id AND t.status = 'Resolved/Closed'
                      AND t.closed_at BETWEEN ? AND ? AND c.name != ?) AS closed_count,
                 (SELECT COALESCE(SUM(sl.labor_fee), 0) FROM Service_Logs sl
                    JOIN Tickets t2 ON t2.ticket_id = sl.ticket_id
                    JOIN Devices d2 ON d2.device_sn = t2.device_sn
                    JOIN Customers c2 ON c2.customer_id = d2.customer_id
                    WHERE t2.assigned_tech_id = u.user_id AND sl.created_at BETWEEN ? AND ?
                          AND c2.name != ?) AS labor_total
               FROM Users u WHERE u.role IN ('technician','technician_lead') AND u.is_active = 1 AND u.center_id = ?
               ORDER BY u.name""",
            (from_sql, to_sql, EXCLUDED_CASE_COUNT_CUSTOMER_NAME,
             from_sql, to_sql, EXCLUDED_CASE_COUNT_CUSTOMER_NAME,
             from_sql, to_sql, EXCLUDED_CASE_COUNT_CUSTOMER_NAME, scope_center),
        ).fetchall()
        ...
        tech_rows = conn.execute(
            """SELECT u.user_id, u.name,
                 ...
                 (SELECT COALESCE(SUM(sl.labor_fee), 0) FROM Service_Logs sl
                    ...
                    WHERE t2.assigned_tech_id = u.user_id AND sl.created_at BETWEEN ? AND ?
                          AND c2.name != ?) AS labor_total
               FROM Users u WHERE u.role IN ('technician','technician_lead') AND u.is_active = 1
               ORDER BY u.name""",
            (from_sql, to_sql, EXCLUDED_CASE_COUNT_CUSTOMER_NAME,
             from_sql, to_sql, EXCLUDED_CASE_COUNT_CUSTOMER_NAME,
             from_sql, to_sql, EXCLUDED_CASE_COUNT_CUSTOMER_NAME),
        ...
        assigned_case_rows = conn.execute(
            f"""SELECT t.ticket_id, t.assigned_tech_id, d.model, t.status, t.created_at
                ...
                WHERE t.assigned_tech_id IN ({placeholders}) AND t.created_at BETWEEN ? AND ?{center_cond}
                      AND c.name != ?
                ORDER BY t.created_at DESC""",
            (*tech_ids, from_sql, to_sql, *center_params, EXCLUDED_CASE_COUNT_CUSTOMER_NAME),
        ).fetchall()
        ...
        closed_case_rows = conn.execute(
            f"""SELECT t.ticket_id, t.assigned_tech_id, d.model, t.closed_at
                ...
                WHERE t.assigned_tech_id IN ({placeholders}) AND t.status = 'Resolved/Closed'
                  AND t.closed_at BETWEEN ? AND ?{center_cond} AND c.name != ?
                ORDER BY t.closed_at DESC""",
            (*tech_ids, from_sql, to_sql, *center_params, EXCLUDED_CASE_COUNT_CUSTOMER_NAME),
        ).fetchall()
        ...
        labor_case_rows = conn.execute(
            f"""SELECT sl.ticket_id, t.assigned_tech_id, d.model, sl.action_taken, sl.labor_fee, sl.created_at
                ...
                WHERE t.assigned_tech_id IN ({placeholders}) AND sl.labor_fee > 0
                  AND sl.created_at BETWEEN ? AND ?{center_cond} AND c.name != ?
                ORDER BY sl.created_at DESC""",
            (*tech_ids, from_sql, to_sql, *center_params, EXCLUDED_CASE_COUNT_CUSTOMER_NAME),
        ).fetchall()
...
def _commission_period_nav(year, month, half):
    """คำนวณงวดก่อนหน้า/ถัดไป จากงวดปัจจุบัน (year, month, half) — ใช้ทำปุ่มเลื่อนงวดบนหน้ารายงานค่าคอมมิชชั่น"""
    if half == 1:
        prev = (year - 1, 12, 2) if month == 1 else (year, month - 1, 2)
        nxt = (year, month, 2)
    else:
        prev = (year, month, 1)
        nxt = (year + 1, 1, 1) if month == 12 else (year, month + 1, 1)
    return prev, nxt


@route("GET", r"/admin/commission-report")
def admin_commission_report(environ, m, conn, user):
    """รายงานค่าคอมมิชชั่นพิเศษของช่าง/หัวหน้าช่าง ต่อรอบบิล (ครึ่งเดือน) — สำหรับ admin (ดูได้ทุกศูนย์/กรองศูนย์ได้)
    manager และ technician_lead (ดูเฉพาะทีมช่างในศูนย์ของตัวเอง)"""
    require_login(user, "admin", "manager", "technician_lead")
    from_sql, to_sql, period_label, year, month, half = commission_period_range(environ)
    is_scoped = user["role"] in ("manager", "technician_lead")

    qs = parse_qs(environ.get("QUERY_STRING", ""))
    selected_center_raw = qs.get("center_id", [""])[0].strip()
    selected_center_id = int(selected_center_raw) if selected_center_raw.isdigit() else None
    scope_center = user.get("center_id") if is_scoped else selected_center_id
    ...
    rows = _technician_commission_rows(conn, from_sql, to_sql, center_id=scope_center)
    total_commission = round(sum(r["total_commission"] for r in rows), 2)
    total_bonus_cases = sum(r["bonus_case_count"] for r in rows)
    total_closed_cases = sum(r["closed_count"] for r in rows)

    (prev_year, prev_month, prev_half), (next_year, next_month, next_half) = _commission_period_nav(year, month, half)

    return render(
        "admin_commission_report.html", user=user, rows=rows, period_label=period_label,
        year=year, month=month, half=half,
        prev_year=prev_year, prev_month=prev_month, prev_half=prev_half,
        next_year=next_year, next_month=next_month, next_half=next_half,
        total_commission=total_commission, total_bonus_cases=total_bonus_cases,
        total_closed_cases=total_closed_cases, bonus_case_rank=COMMISSION_BONUS_CASE_RANK,
        bonus_rate_percent=int(COMMISSION_BONUS_RATE * 100),
        all_centers=all_centers, centers_for_js=centers_for_js,
        selected_center_id=selected_center_id, selected_center_name=selected_center_name,
        is_scoped=is_scoped,
    )


@route("GET", r"/admin/commission-report/(\d+)/bill/print")
def commission_bill_print(environ, m, conn, user):
    """บิลแจ้งค่าคอมมิชชั่นพิเศษของช่าง/หัวหน้าช่างคนหนึ่ง สำหรับงวดที่เลือก (พิมพ์/บันทึกเป็น PDF ได้) —
    admin ดูได้ทุกคน, manager/technician_lead ดูได้เฉพาะทีมช่างในศูนย์ของตัวเอง (รวมบิลของตัวเอง),
    ส่วนช่าง/หัวหน้าช่างทั่วไป ดูได้เฉพาะบิลของตัวเองเท่านั้น"""
    require_login(user, "admin", "manager", "technician", "technician_lead")
    target_id = int(m.group(1))
    tech = conn.execute(
        "SELECT user_id, name, role, center_id FROM Users WHERE user_id=? AND role IN ('technician','technician_lead')",
        (target_id,),
    ).fetchone()
    if not tech:
        raise HttpError(404, "ไม่พบช่าง/หัวหน้าช่างรายนี้")
    if user["role"] == "technician":
        if user["user_id"] != target_id:
            raise HttpError(403, "คุณดูได้เฉพาะบิลค่าคอมมิชชั่นของตัวเองเท่านั้น")
    elif user["role"] in ("manager", "technician_lead"):
        if user["user_id"] != target_id:
            require_center_access(user, tech["center_id"])
    # admin ดูได้ทุกคนไม่จำกัด

    from_sql, to_sql, period_label, year, month, half = commission_period_range(environ)
    rows = _technician_commission_rows(conn, from_sql, to_sql, tech_user_id=target_id)
    if not rows:
        raise HttpError(404, "ไม่พบข้อมูลค่าคอมมิชชั่นของช่างรายนี้")
    row = rows[0]
    center = conn.execute("SELECT * FROM Service_Centers WHERE center_id=?", (tech["center_id"],)).fetchone()
    hq = _get_hq_center(conn)
    return render(
        "commission_bill_print.html", user=user, row=row, tech=tech, center=center, hq=hq,
        period_label=period_label, year=year, month=month, half=half,
        bonus_case_rank=COMMISSION_BONUS_CASE_RANK, bonus_rate_percent=int(COMMISSION_BONUS_RATE * 100),
    )
...
def quote_print(environ, m, conn, user):
    """ใบเสนอราคาสำหรับตั๋วซ่อม — รองรับ checkbox "รวม VAT" ส่งมาเป็น query string ?vat=1 (GET form, ไม่ต้องใช้ JS)
    เหมือนกับใบเสนอราคาฝั่งขายสินค้า (sales_quote_print) — ถ้าไม่เลือก จะไม่คำนวณ/ไม่แสดงแถวภาษีมูลค่าเพิ่มเลย"""
    ...
    return render("quote_print.html", q=q, t=t, items=items, subtotal=subtotal, vat=vat, grand_total=grand_total,
                   include_vat=include_vat, vat_rate_percent=int(db.VAT_RATE * 100), hq=hq,
...
def invoice_print(environ, m, conn, user):
    """ใบแจ้งหนี้ค่าซ่อม — รองรับ checkbox "รวม VAT" ผ่าน query string ?vat=1 เหมือนใบเสนอราคา
    ถ้าไม่เลือก จะแสดงยอดรวมสุทธิตรงๆ แบบเดิม (ไม่มีแถวภาษีมูลค่าเพิ่ม)"""
    ticket_id = int(m.group(1))
    if not user_can_view_ticket(conn, user, ticket_id):
    ...
    return render("invoice_print.html", t=t, invoice_items=invoice_items, invoice_total=invoice_total,
                   subtotal=subtotal, vat=vat, grand_total=grand_total, include_vat=include_vat,
                   vat_rate_percent=int(db.VAT_RATE * 100))
...
def receipt_print(environ, m, conn, user):
    """ใบเสร็จรับเงินค่าซ่อม — รองรับ checkbox "รวม VAT" ผ่าน query string ?vat=1 เช่นเดียวกับใบเสนอราคา/ใบแจ้งหนี้
    ถ้าเลือก จะออกเป็น "ใบกำกับภาษี" (แสดงแถวภาษีมูลค่าเพิ่ม) แทนที่ "ใบเสร็จรับเงิน/บิลเงินสด" แบบไม่มี VAT ตามค่าเริ่มต้น"""
    ticket_id = int(m.group(1))
    if not user_can_view_ticket(conn, user, ticket_id):
    ...
    return render("receipt_print.html", t=t, invoice_items=invoice_items, invoice_total=invoice_total,
                   payments=confirmed, total_paid=total_paid, subtotal=subtotal, vat=vat, grand_total=grand_total,
                   include_vat=include_vat, vat_rate_percent=int(db.VAT_RATE * 100))
...
def serve_device_proof(path, conn, user, start_response):
    ...
    allowed = False
    if device and user:
        if user["role"] in ("admin", "manager", "technician", "technician_lead"):
            allowed = True
