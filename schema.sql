...
CREATE TABLE IF NOT EXISTS Users (
    ...
    role         VARCHAR(20) NOT NULL,               -- 'customer','admin','technician','technician_lead','manager','sales'
    name         TEXT NOT NULL,
    ...
    CONSTRAINT chk_users_role CHECK (role IN ('customer','admin','technician','technician_lead','manager','sales')),
...
CREATE TABLE IF NOT EXISTS Restock_Orders (
    ...
    shipped_at        TEXT,
    received_at       TEXT,
    payment_confirmed_at TEXT,     -- วันที่ HQ ยืนยันว่าศูนย์บริการชำระค่าสินค้าคำสั่งซื้อนี้แล้ว (NULL = ยังไม่ยืนยัน/
                                    -- ยังไม่ออกใบกำกับภาษี) — แยกอิสระจากสถานะจัดส่ง (status) เพราะรับของแล้วไม่ได้แปลว่าจ่ายแล้วเสมอไป
    payment_confirmed_by INT,      -- Users.user_id ของแอดมิน (HQ) ที่กดยืนยันรับชำระ
    FOREIGN KEY (center_id) REFERENCES Service_Centers(center_id),
